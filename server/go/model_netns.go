/*
 * netConfD API
 *
 * Network Configurator service
 *
 * API version: 0.1.0
 * Contact: support@athonet.com
 */

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// Netns Network Namespace
type Netns struct {
	Name *string `json:"name,omitempty"`
}

// NewNetns instantiates a new Netns object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewNetns() *Netns {
	this := Netns{}
	return &this
}

// NewNetnsWithDefaults instantiates a new Netns object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewNetnsWithDefaults() *Netns {
	this := Netns{}
	return &this
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *Netns) GetName() string {
	if o == nil || o.Name == nil {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *Netns) GetNameOk() (*string, bool) {
	if o == nil || o.Name == nil {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *Netns) HasName() bool {
	if o != nil && o.Name != nil {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *Netns) SetName(v string) {
	o.Name = &v
}

func (o Netns) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if o.Name != nil {
		toSerialize["name"] = o.Name
	}
	return json.Marshal(toSerialize)
}

type NullableNetns struct {
	value *Netns
	isSet bool
}

func (v NullableNetns) Get() *Netns {
	return v.value
}

func (v *NullableNetns) Set(val *Netns) {
	v.value = val
	v.isSet = true
}

func (v NullableNetns) IsSet() bool {
	return v.isSet
}

func (v *NullableNetns) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableNetns(val *Netns) *NullableNetns {
	return &NullableNetns{value: val, isSet: true}
}

func (v NullableNetns) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableNetns) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


